...
### A2 - Install a Coldbox `merapi` server app
...
Now that we are in the cbox3 folder, all commands assume that we are 
in the `commandbox` shell (unless stated otherwise). 
Find all Coldbox application templates here: https://github.com/coldbox-templates
...
Run the command: `coldbox create app name=merapi skeleton=rest`
...
`this.datasource = "merapi";`

### B4 - Update the dot env file

A dot env file is needed to allow seamless access to the `merapi` database instance schema, without leaking database access credentials outside the local development environment (make sure this file is added to dot gitignore file). 
The dot env file uses "coldbox" as the default database instance, which means you need to replace this name with "merapi", or whatever name you used for your environment and database instance.
...
DB_CONNECTIONSTRING=jdbc:mysql://127.0.0.1:3306/merapi?useSSL=false&useUnicode=true&characterEncoding=UTF-8&serverTimezone=UTC&useLegacyDatetimeCode=true
...
successfully to a `merapi` instance.
...
You should have this module available if you performed the template installation steps described in paragraph A3.

Otherwise, if you can't find it, proceed with the module installation: `install commandbox-cfconfig`.

Let's set a password for the Lucee administrator: `cfconfig set adminPassword=merapi`
...
The "merapi" datasource should have already been created by the Lucee engine based on the .cfconfig.json file 
settings that are read when the server re-starts. The .cfconfig.json reads its database connection parameters from the dot env file. Test the datasource connection.

If the test fails, delete and re-create a new datasource with the name `merapi` and with a driver type of `mysql`. 
It should now work. The problem arose from a mis-match in the jdbc class driver name. The default .env file generated by the Coldbox API template uses a deprecated MySQL driver class.

If you choose a name other than "merapi" for the datasource, remember to update Application.cfc accordingly (as per paragraph B-3).
Stop and re-start your server for the changes to take effect. 
...
Check whether `box.json` includes a **cfmigrations** structure. If it does not, run `migrate init` to create this structure.
...
For this to work, you need a line with `DB_SCHEMA=merapi` in your dot env file, otherwise you may get a misleading message such as "cfmigrations table is already installed" while, in fact, it is not.
...
> - migrate up // Commit all migration files (found in resources/database/migrations) to the database schema
...
Go to the `merapi` folder and have a look at its content. It should now include:

```sh

coldbox                         // Coldbox framework installation
config                          // application config directory
frontend                        // The ReactJS project that builds our UI
...
Open `http://localhost:52019/tests` in your browser
